...
function applyZoneDefaults(payload, log)
{
    ...
    if (BRAND_OPTIONS[payload.brand].features.disable_paging) {
        if (!payload.hasOwnProperty('disable_paging')) {
            payload.disable_paging = false;
        }
    }
...
function buildZonecfgUpdate(vmobj, payload, log)
{
        ...
        if (payload.hasOwnProperty(attr_name)) {
            if (remove_attr) {
                ...
                if (attr_name === 'resolvers'
                    ...
                    // XXX: there is likely a better way to detect if the value
                    // type is boolean, then this will work for any future
                    // boolean attrs.
                    if (attr_name === 'disable_paging') {
                        zcfg = zcfg + 'set type=boolean; ';
                    } else {
                        zcfg = zcfg + 'set type=string; ';
                    }
                }
    ...
    if (BRAND_OPTIONS[brand].features.disable_paging) {
        setAttr('phys-mcap-no-pageout', 'disable_paging');
    }
